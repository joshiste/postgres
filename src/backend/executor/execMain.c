 ...
 *
 * Receivers that insert the rows into a table (DestIntoRel, DestTransientRel)
 * are left out although they store nothing themselves: a compressed value
 * arriving detoasted would have to be compressed again on the way in, where
 * the toast pointer lets heap_toast_insert_or_update copy the compressed
 * bytes as they are.
 */
static bool
